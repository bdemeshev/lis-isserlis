...
Немного заранее подготовимся! Во-первых, $q(0) = 0$, $\exp(q(0)) = 1$.
...
Вторая производная $q''_{12}(u) = c_{12}$ равна соответствующей ковариации. 

Третья производная $q'''_{123} = 0$, так как $q(u)$ — квадратичная функция.
...
Продолжаем,
\[
    \E(Y_1 Y_2 Y_3) = \mgf'''_{123} = \exp(q) (q'_1 q'_2 q'_3 + q'_3 c_{12} + q'_1 c_{23} + q'_2 c_{13}) = \mu_1 \mu_2\mu_3 + \mu_1 c_{23} + \mu_2 c_{13} + \mu_3 c_{12}.
\]
Ответ легко описать словами.
Каждое слагаемое идёт с коэффициентом $1$.
Каждое слагаемое содержит все индексы от 1 до 3 ровно по одному разу. 
Все варианты включения $\mu_i$ и $c_{ij}$ присутствуют. 
Множитель $\mu_i$ «съедает» один индекс, а $c_{ij}$ «съедает» два индекса. 

Если какой-то индекс повторяется, то его надо повторить :)
\[
    \E(Y_1 Y_2 Y_2) = \mgf'''_{122} = \exp(q) (q'_1 q'_2 q'_2 + q'_2 c_{12} + q'_1 c_{22} + q'_2 c_{12}) = \mu_1 \mu_2\mu_2 + \mu_1 c_{22} + \mu_2 c_{12} + \mu_2 c_{12}.
\]
